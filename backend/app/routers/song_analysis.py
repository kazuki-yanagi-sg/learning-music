"""
楽曲解析API（YouTube + yt-dlp + Magenta + Gemini）
...
async def get_song_analysis_info():
    ...
    return {
        "success": True,
        "data": {
            "message": "楽曲解析API（YouTube + yt-dlp + Magenta + Gemini）",
            "endpoints": {
                "/search": "曲を検索（クエリ必須）",
...
async def analyze_with_progress(video_id: str, generate_ai_analysis: bool = True) -> AsyncGenerator[str, None]:
    ...
    try:
        ...
        # 3. Magentaで音声をMIDIに変換
        yield send_event("convert", 45, "MIDI変換中...")
        await asyncio.sleep(0)
        ...
        if not midi_result["success"]:
            yield send_event("error", 0, f"MIDI変換エラー: {midi_result['error']}")
            return

        midi_path = midi_result["midi_path"]
        yield send_event("convert", 60, "MIDI変換完了")
        await asyncio.sleep(0)

        # 4. MIDIを解析
        yield send_event("analyze", 65, "MIDI解析中...")
        await asyncio.sleep(0)

        parse_result = magenta.parse_midi(midi_path)

        if not parse_result["success"]:
            yield send_event("error", 0, f"MIDI解析エラー: {parse_result['error']}")
            return
        ...
        # 5. コード進行を抽出
        chords_data = magenta.extract_chords_from_notes(parse_result["notes"])
        ...
        if generate_ai_analysis and chords:
            ...
            try:
                ...
                analysis_text = await gemini.generate_song_analysis(
                    ...
                    chords=chord_list,
                    notes_count=len(parse_result["notes"]),
                )
            except Exception as e:
...
async def analyze_video(video_id: str, generate_ai_analysis: bool = True):
    """
    曲を解析する（yt-dlp → Magenta → コード認識 → AI解説）
    ...
    try:
        ...
        if not midi_result["success"]:
            raise HTTPException(
                status_code=500,
                detail=f"MIDI変換エラー: {midi_result['error']}"
            )
        ...
        if not parse_result["success"]:
            raise HTTPException(
                status_code=500,
                detail=f"MIDI解析エラー: {parse_result['error']}"
            )
