...
class LibrosaTranscriber:
    ...
    def extract_drums(self, audio_path: str, tempo: float = None) -> dict:
        ...
        try:
            ...
            logger.debug(f"[Librosa] Drum audio loaded: {len(y)} samples, sr={sr}, duration={len(y)/sr:.1f}s")
            ...
            logger.debug(f"[Librosa] After classification: {len(notes)} notes")
